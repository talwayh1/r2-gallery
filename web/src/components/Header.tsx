...
export default function Header({
  ...
  // Check if current filters deviate from defaults — used for visual badge on mobile
  const hasActiveFilters = sortBy !== 'name' || typeFilter !== 'all';
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        {isMobile && (
          <>
            ...
            <button onClick={() => setMobileFilterOpen(!mobileFilterOpen)} className={`p-1.5 rounded-lg shrink-0 transition-colors relative ${mobileFilterOpen ? 'bg-blue-500/20 text-blue-400' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`} title="筛选">
              {hasActiveFilters && (
                <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-gray-800 z-10" />
              )}
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>

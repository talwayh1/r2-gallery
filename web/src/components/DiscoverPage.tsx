            ...
            return (
              ...
              >
                {isImage ? (
                  <>
                    {/* Shimmer placeholder */}
                    {!isLoaded && !failedImages.has(file.path) && (
                      <div className="shimmer w-full aspect-square" />
                    )}
                    {/* Fallback when image fails to load */}
                    {failedImages.has(file.path) ? (
                      <div className="w-full aspect-square flex items-center justify-center bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800">
                      ...
                      <img
                        ...
                        onError={() => setFailedImages(prev => new Set(prev).add(file.path))}
                      />
                    )}
                  </>
                ) : isVideo ? (
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-white/60 text-[10px]">
                      {file.dir && <span
                        className="inline-flex items-center gap-1 hover:text-white cursor-pointer transition-colors"
                        onClick={(e) => { e.stopPropagation(); onNavigate(file.dir); }}
                      ><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>{file.dir}</span>}

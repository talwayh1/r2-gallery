...
export default function Lightbox({ items, index, onClose, onNavigate, onDelete, onDuplicate, onLocate, onSelect, selectedPaths, closing }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* File info bar */}
      <div className={`absolute top-4 left-4 text-white/70 text-sm z-10 flex items-center gap-3 transition-opacity duration-200 ${!uiVisible && (isImage || isVideo) ? 'opacity-0 pointer-events-none' : ''} ${isMobile ? 'max-w-[55%]' : ''}`}>
        <span className={`font-medium text-white/90 ${isMobile ? 'truncate flex-1 min-w-0' : ''}`} title={isMobile ? name : undefined}>{name}</span>
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg transition-colors text-xs ${
                directUrlCopied ? 'bg-green-500/20 text-green-400' : 'bg-white/5 hover:bg-white/10 text-white/50'
              }`}
            >
              {directUrlCopied ? (
                <>
